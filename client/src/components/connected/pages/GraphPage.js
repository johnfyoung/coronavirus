...
class GraphPage extends Component {
  ...
  componentDidMount() {
    ...
    const newsURL = `https://cac-cors.herokuapp.com/https://news.google.com/rss/search?q=${county}+county+${state}+coronavirus&hl=en-US&gl=US&ceid=US:en`;
          ...
          <div className="col-12">
            {match && match.params ? (
              <Fragment>
                <h1>
                  Stats for{" "}
                  {match.params.county.charAt(0).toUpperCase() +
                    match.params.county.slice(1)}{" "}
                  County (
                  {match.params.state.charAt(0).toUpperCase() +
                    match.params.state.slice(1)}{" "}
                  State)
                </h1>
                          ...
                          {this.state.news.map((newsItem, i) => (
                            <li key={i}>
                              <a href={newsItem.link} target="_blank">
                                {newsItem.title} ({newsItem.pubDate})
                              </a>{" "}
                            </li>
                          ))}
                        </ul>
...
export default connect(mapStateToProps, actionCreators)(GraphPage);
